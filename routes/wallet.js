...
let { createWalletValidator, deleteWalletValidator, updateWalletValidator, viewWalletValidator, validateApp } = require('../validators/wallet-validator')
...
router.get('/view-wallet/:wallet_id', validate, viewWalletValidator(), validateApp, async (req,res) => {
    try {
        let wallet = await Wallet.findOne({ user_id: req.user_id, _id: req.params.wallet_id }, 'name type balance status').populate('color_id', 'code')
        res.status(200).json({ status: true, wallet: wallet})
    } catch (error) {
        console.log(error);
        res.status(500).json({ status: false, error: error.message, message: "Something went wrong" })
    }
